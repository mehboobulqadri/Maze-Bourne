...
from src.core.constants import SCREEN_WIDTH, SCREEN_HEIGHT, COLORS, GameState
...
class SettingsScreen(Screen):
    ...
    def on_enter(self):
        ...
        # Difficulty Button
        current_diff = sm.get("game", "difficulty") or "normal"
        
        def toggle_difficulty():
             # Re-fetch sm here or bind it
             s_mgr = self.manager.game.settings_manager
             curr = s_mgr.get("game", "difficulty") or "normal"
             ...
             s_mgr.set("game", "difficulty", new_mode)
             
             # Update button text
             self.btn_difficulty.text = f"DIFFICULTY: {new_mode.upper()}"
             # Re-render text
             self.btn_difficulty.text_surf = self.btn_difficulty.font.render(self.btn_difficulty.text, True, UITheme.COLOR_TEXT_PRIMARY)
             self.btn_difficulty.text_rect = self.btn_difficulty.text_surf.get_rect(center=self.btn_difficulty.rect.center)
             
        self.btn_difficulty = UIButton(self.center_x - 150, 360, 300, 50, f"DIFFICULTY: {current_diff.upper()}", toggle_difficulty, font_med)
        self.elements.append(self.btn_difficulty)
        # Update sliders
...
class MainMenuScreen(Screen):
    ...
    def create_buttons(self, x, start_y, gap, w, h):
        ...
        def go_settings():
             # Use change_state to ensure game state update and UI switching happen in sync
             self.manager.game.change_state(GameState.SETTINGS)
             
        def go_credits():
             self.manager.game.change_state(GameState.CREDITS)
